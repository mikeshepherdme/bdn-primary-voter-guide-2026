...
function topicBodyHtml(td) {
  ...
  // Fallback for topics not yet synthesized: non-video quotes only
  const quotes = td.quotes || [];
  const sortedAll = quotes.filter(q => typeof q === 'object' && !q.embed_url).slice().sort((a, b) => quoteScore(a) - quoteScore(b));
  return `

...
function topicBodyHtml(td) {
  if (td.synthesis) {
    ...
    return bulletsHtml ? `<ul class="synthesis-bullets">${bulletsHtml}</ul>` : '';
  }

  // Fallback for topics not yet synthesized
  const quotes = td.quotes || [];
  const sortedAll = quotes.filter(q => typeof q === 'object').slice().sort((a, b) => quoteScore(a) - quoteScore(b)).slice(0, 2);
  return `
    ${td.position ? `<div class="topic-position-text">${td.position}</div>` : ''}
    ${sortedAll.length > 0 ? `<div class="quotes-list">${sortedAll.map(q => quoteItemHtml(q)).join('')}</div>` : ''}`;
}
...
function quoteItemHtml(q) {
  ...
  if (embedUrl) {
    // Render as plain text quote linking directly to the timestamped video, no embed player
    const watchUrl = videoUrl || (embedUrl.replace('youtube.com/embed/', 'youtube.com/watch?v=').replace(/\?start=(\d+)/, (_, s) => `&t=${s}`));
    const tsLink = ts != null ? ` at ${formatTs(ts)}` : '';
    const safeWatch = watchUrl.replace(/"/g, '%22');
    const linkedSource = `<div class="quote-source-line"><a href="${safeWatch}" target="_blank" rel="noopener">${source}${tsLink}</a></div>`;
    return `
      <div class="quote-item q-direct">
        <div class="quote-type-row"><span class="badge badge-direct">Direct quote</span></div>
